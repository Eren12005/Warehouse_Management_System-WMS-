...
class SupplierManager:
    ...
    def view_suppliers(self):
        ...
        print("-" * 75)
        for s in suppliers:
            print(f"{s['supplier_id']:<5} | {s['supplier_name']:<25} | {str(s['phone']):<15} | {str(s['email']):<25}")
...
class ProductManager:
    ...
    def add_product(self):
        ...
        location = get_non_empty_string("Warehouse Location (e.g. A1, B2): ")
    ...
    def view_products(self):
        ...
        print(f"{'ID':<5} | {'Name':<20} | {'Category':<15} | {'Price':<10} | {'Qty':<6} | {'Loc':<8} | {'Supplier':<15}")
        print("-" * 88)
        for p in products:
            sup_name = p['supplier_name'] or "N/A"
            print(f"{p['product_id']:<5} | {p['product_name']:<20} | {p['category']:<15} | ${p['price']:<9.2f} | {p['quantity']:<6} | {p['warehouse_location']:<8} | {sup_name:<15}")
    ...
    def search_product(self):
        ...
        print("-" * 65)
        for p in results:
            print(f"{p['product_id']:<5} | {p['product_name']:<20} | {p['category']:<15} | ${p['price']:<9.2f} | {p['quantity']:<6}")
    ...
    def update_product(self):
        ...
        location = input(f"New Location [{existing['warehouse_location']}]: ").strip() or existing['warehouse_location']
...
class WarehouseManager:
    ...
    def assign_location(self):
        ...
        new_loc = get_non_empty_string(f"Current Location [{product['warehouse_location']}]. New Location: ")
    ...
    def view_by_location(self):
        ...
        loc = get_non_empty_string("Enter Location/Section (e.g. A1): ")
        query = "SELECT * FROM Products WHERE warehouse_location = %s"
        products = self.db.fetch_all(query, (loc,))

        if not products:
            print(f"No products found at location '{loc}'.")
            return

        print(f"{'ID':<5} | {'Product Name':<20} | {'Quantity':<6}")
        print("-" * 40)
        for p in products:
            print(f"{p['product_id']:<5} | {p['product_name']:<20} | {p['quantity']:<6}")
...
class OrderManager:
    ...
    def view_orders(self):
        ...
        print(f"{'ID':<5} | {'Customer':<20} | {'Product':<15} | {'Qty':<5} | {'Status':<12} | {'Date':<19}")
        print("-" * 85)
        for o in orders:
            print(f"{o['order_id']:<5} | {o['customer_name']:<20} | {o['product_name']:<15} | {o['quantity']:<5} | {o['status']:<12} | {str(o['order_date']):<19}")
...
class ReportManager:
    ...
    def low_stock_report(self, threshold=5):
        ...
        print(f"{'ID':<5} | {'Product Name':<25} | {'Quantity':<10} | {'Location':<10}")
        print("-" * 58)
        for r in results:
            print(f"{r['product_id']:<5} | {r['product_name']:<25} | {r['quantity']:<10} | {r['warehouse_location']:<10}")
    ...
    def transaction_history_report(self):
        ...
        print(f"{'ID':<5} | {'Product':<20} | {'Stock In':<10} | {'Stock Out':<10} | {'Timestamp':<19}")
        print("-" * 72)
        for l in logs:
            print(f"{l['stock_id']:<5} | {l['product_name']:<20} | {l['stock_in']:<10} | {l['stock_out']:<10} | {str(l['transaction_date']):<19}")
...
class Dashboard:
    ...
    def warehouse_menu(self):
        while True:
            print("\n--- WAREHOUSE LOCATION MENU ---")
            print("1. View Products at Location")
